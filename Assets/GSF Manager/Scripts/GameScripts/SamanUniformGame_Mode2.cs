...
[System.Serializable]
public class SamanUniformGame_Mode2_UI
{
    ...
    public GameObject BeachthingScroller,NecklaceScroller, HairScroller, GlassesScroller, BagScroller, AllScrollers;
    [Header("PLayer Items")]
    public Image PlayerDress;
    public Image PlayerBeachthing,PlayerNecklace, PlayerHair, PlayerGlasses, PlayerBag;
    public Image playerAvatar_1, playerAvatar_2;
    [Header("Opponent Items")]
    public Image OpponentDress;
    public Image OpponentBeachthing,OpponentNecklace, OpponentHair,  OpponentGlasses, OpponentBag;
...
public class SamanUniformGame_Mode2 : MonoBehaviour
{
    #region Variables
    [System.Serializable]
    public enum SelectedItem
    {
        None, Dress,Bag, Hair, Beachthing, Glasses, Necklace,
        
    }
    [FoldoutGroup("IQRA INDIAN SGAME INDIAN WEDDING UI")]
    [HideLabel]
    public SamanUniformGame_Mode2_UI uIElements;
    private SelectedItem selectedItem;
    public Sprite[] PlayerAvatars;
    public Sprite[] opponentAvatars;
    public Sprite[] HairSprites;
    public Sprite[] DressSprites;
    public Sprite[] Glassesprites;
    public Sprite[] BeachthingSprites;
    ...
    private List<ItemInfo> BeachthingList = new List<ItemInfo>();
    ...
    private int necklaceRank, GlassesRank,Beachthingrank;
    ...
    private void Start()
    {
        ...
        BagRank = dressRank = hairsRank = GlassesRank = Beachthingrank = 1;
    ...
    IEnumerator SamanUniformGame_InitializeValues()
    {
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode2Items.DressLock, dressList);
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode2Items.HairLock, hairList);
        ...
        #region Initializing Beachthing
        if (uIElements.BeachthingScroller)
        {
            var BeachthingInfo = uIElements.BeachthingScroller.GetComponentsInChildren<ItemInfo>();
            for (int i = 0; i < BeachthingInfo.Length; i++)
            {
                BeachthingList.Add(BeachthingInfo[i]);
            }
        }
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode2Items.BeachthingLock, BeachthingList) ;
        SamanUniformGame_SetItemIcon(BeachthingList,BeachthingSprites);
        #endregion
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode2Items.NecklaceLock, necklaceList);
        ...
        #region Initializing Glasses
        if (uIElements.GlassesScroller)
        {
            var noseRingsInfo = uIElements.GlassesScroller.GetComponentsInChildren<ItemInfo>();
            for (int i = 0; i < noseRingsInfo.Length; i++)
            {
                GlassesList.Add(noseRingsInfo[i]);
            }
        }
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode2Items.GlassesLock, GlassesList);
        SamanUniformGame_SetItemIcon(GlassesList,Glassesprites);
        #endregion
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode2Items.BagLock, BagList);
    ...
    private void SamanUniformGame_GetItemsInfo()
    {
        ...
        else if (selectedItem == SelectedItem.Beachthing)
        {
            SamanUniformGame_SetItemsInfo(BeachthingList);
        }
    
      
       
        else if (selectedItem == SelectedItem.Glasses)
        {
            SamanUniformGame_SetItemsInfo(GlassesList);
        }
    ...
    public void SamanUniformGame_SelectItem(int index)
    {
        ...
        {
            SamanUniformGame_CheckSelectedItem(BeachthingList, BeachthingSprites, uIElements.PlayerBeachthing);
        }
       
        else if (selectedItem == SelectedItem.Glasses)
        {
            SamanUniformGame_CheckSelectedItem(GlassesList, Glassesprites, uIElements.PlayerGlasses);
        }
    ...
    public void SamanUniformGame_UnlockSingleItem()
    {
        if (selectedItem == SelectedItem.Hair)
        {
            SaveData.Instance.Mode2Items.HairLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Beachthing)
        {
            SaveData.Instance.Mode2Items.BeachthingLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Glasses)
        {
            SaveData.Instance.Mode2Items.GlassesLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Dress)
        {
            SaveData.Instance.Mode2Items.DressLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Necklace)
        {
            SaveData.Instance.Mode2Items.NecklaceLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Bag)
        {
            SaveData.Instance.Mode2Items.BagLock[selectedIndex] = false;
        }
        SamanGame_SaveLoad.SaveProgress();
    }
    #endregion
    ...
    public void SamanUniformGame_SelectCategory(int index)
    {
        ...
        if (index == 0)
        {
            ...
            selectedItem = SelectedItem.Dress;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-10f, -224f, 0f), 1);
        }
        else if (index == 1)
        {
            uIElements.HairScroller.SetActive(true);
            selectedItem = SelectedItem.Hair;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-10f, -180f, 0f), 1);
        }
        else if (index == 2)
        {
            uIElements.NecklaceScroller.SetActive(true);    
            selectedItem = SelectedItem.Necklace;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(0f, -500f, -200f), 1);
        }
        else if (index == 3)
        {
            uIElements.GlassesScroller.SetActive(true);
            selectedItem = SelectedItem.Glasses;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-100f, -500f, -300f), 1);
        }
        else if (index == 4)
        {
            uIElements.BagScroller.SetActive(true);
            selectedItem = SelectedItem.Bag;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-40f, -50f, 350f), 1);
        }
      
        else if (index == 5)
        {
            uIElements.BeachthingScroller.SetActive(true);
            selectedItem = SelectedItem.Beachthing;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-10f, -50f, 300f), 1);
        }
        SamanUniformGame_GetItemsInfo();
    ...
    public void SamanUniformGame_DisableScroller()
    {
        ...
        uIElements.BeachthingScroller.SetActive(false);
    ...
    public void SamanUniformGame_Submit()
    {
        uIElements.coinSlot.SetActive(false);
        uIElements.videoSlot.SetActive(false);
        uIElements.AllScrollers.SetActive(false); uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-250f, -50f, 500f), 0.5f).OnComplete(delegate
        {
    ...
    IEnumerator SamanUniformGame_Judgement()
    {
        ...
        uIElements.OpponentCharacter.transform.localPosition += Vector3.right * 1100;
        uIElements.OpponentCharacter.transform.parent.gameObject.SetActive(true);
        uIElements.OpponentCharacter.transform.DOLocalMove(new Vector3(500f, -50f, 500f), 0.5f);
        yield return new WaitForSeconds(0.5f);
        ...
        yield return new WaitForSeconds(2f);
        uIElements.playerPoints.text = ((((dressRank + BagRank + Beachthingrank) * 100) / 20)).ToString();
        uIElements.opponentPoints.text = OP_DressingPoints.ToString();
        StartCoroutine(SamanUniformGame_AddPlayerScore((((dressRank + BagRank + Beachthingrank) * 100) / 30)));
        StartCoroutine(SamanUniformGame_AddOpponentScore(OP_DressingPoints));
        ManageStars((((dressRank + BagRank + Beachthingrank) * 100) / 30), OP_DressingPoints);
        yield return new WaitForSeconds(2.5f);
        uIElements.playerPoints.text = ((((GlassesRank + hairsRank + dressRank) * 100) / 20)).ToString();
        uIElements.opponentPoints.text = OP_MakeupPoints.ToString();
        StartCoroutine(SamanUniformGame_AddPlayerScore((((BagRank + hairsRank + dressRank) * 100) / 30)));
        StartCoroutine(SamanUniformGame_AddOpponentScore(OP_MakeupPoints));
        ManageStars((((GlassesRank + hairsRank + dressRank) * 100) / 40), OP_MakeupPoints);
        yield return new WaitForSeconds(2.5f);
        uIElements.playerPoints.text = ((((necklaceRank + GlassesRank) * 100) / 20)).ToString();
        uIElements.opponentPoints.text = OP_JewellertPoints.ToString();
        StartCoroutine(SamanUniformGame_AddPlayerScore((((necklaceRank + GlassesRank) * 100) / 20)));
        StartCoroutine(SamanUniformGame_AddOpponentScore(OP_JewellertPoints));
        ManageStars((((necklaceRank + GlassesRank) * 100) / 20), OP_JewellertPoints);
        yield return new WaitForSeconds(2f);
        ...
        if (playerScore >= opponentScore)
        {
            ...
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(0f, -220f, 100f), 0.5f);
    ...
    private void SamanUniformGame_DressupOpponent()
    {
       ...
       // WearItem(uIElements.OpponentShoes, ShoesSprites);
        ...
        {
            OP_MakeupPoints++;
            WearItem(uIElements.OpponentBeachthing, BeachthingSprites);
        }
        OP_MakeupPoints++;
        WearItem(uIElements.OpponentHair, HairSprites);
        OP_MakeupPoints++;
        WearItem(uIElements.OpponentGlasses, Glassesprites);
    ...
    public void SamanUniformGame_UnlockAllItems()
    {
        ...
        SamanUniformGame_UnlockList(GlassesList);
        SamanUniformGame_UnlockList(necklaceList);
        SamanUniformGame_UnlockList(BagList);
        SamanUniformGame_UnlockList(BeachthingList);
