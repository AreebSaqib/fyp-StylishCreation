...
using DG.Tweening;
#region SamanUniformGame_ModelUI
[System.Serializable]
public class SamanUniformGame_ModelUI
{
    ...
    public GameObject bindiScroller, NecklaceScroller, HairScroller, EarringScroller, BagScroller, AllScrollers;
    [Header("PLayer Items")]
    public Image PlayerDress;
    public Image  PlayerBindi,  PlayerNecklace, PlayerHair, PlayerEarrings, PlayerBag;
    public Image playerAvatar_1, playerAvatar_2;
    [Header("Opponent Items")]
    public Image OpponentDress;
    public Image  OpponentBindi,  OpponentNecklace, OpponentHair, OpponentEarrings, OpponentBag;
...
public class SamanUniformGame_Mode5 : MonoBehaviour
{
    #region Variables
    [System.Serializable]
    public enum SelectedItem
    {
        None, Dress, Bag, Hair, Earrings, Necklace, Bindi

    }
    [FoldoutGroup("IQRA INDIAN SGAME INDIAN WEDDING UI")]
    [HideLabel]
    public SamanUniformGame_ModelUI uIElements;
    ...
    private int necklaceRank, earringsRank, bindiRank;
    ...
    private void Start()
    {
        ...
        BagRank = dressRank = hairsRank = earringsRank = necklaceRank = 1;
    ...
    IEnumerator SamanUniformGame_InitializeValues()
    {
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode5Items.DressLock, dressList);
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode5Items.HairLock, hairList);
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode5Items.BindiLock, bindiList);
        SamanUniformGame_SetItemIcon(bindiList, BindiSprites);
        #endregion
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode5Items.NecklaceLock, necklaceList);
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode5Items.EarringsLock, earringsList);
        ...
        SamanUniformGame_SetupItemData(SaveData.Instance.Mode5Items.BagLock, bagList);
    ...
    private void SamanUniformGame_GetItemsInfo()
    {
        ...
        else if (selectedItem == SelectedItem.Bindi)
        {
            SamanUniformGame_SetItemsInfo(bindiList);
        }
    ...
    public void SamanUniformGame_SelectItem(int index)
    {
        ...
        {
            SamanUniformGame_CheckSelectedItem(bindiList, BindiSprites, uIElements.PlayerBindi);
        }
                    ...
                    if (itemSprites[selectedIndex])
                    {
                        if (itemImage)
                        {
                            ...
                            if (selectedItem == SelectedItem.Bag)
                                ...
                                bindiRank = 1;
                            else if (selectedItem == SelectedItem.Hair)
                                hairsRank = 1;
                            readyCount++;
    ...
    public void SamanUniformGame_UnlockSingleItem()
    {
        if (selectedItem == SelectedItem.Hair)
        {
            SaveData.Instance.Mode5Items.HairLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Bindi)
        {
            SaveData.Instance.Mode5Items.BindiLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Earrings)
        {
            SaveData.Instance.Mode5Items.EarringsLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Dress)
        {
            SaveData.Instance.Mode5Items.DressLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Necklace)
        {
            SaveData.Instance.Mode5Items.NecklaceLock[selectedIndex] = false;
        }
        else if (selectedItem == SelectedItem.Bag)
        {
            SaveData.Instance.Mode5Items.BagLock[selectedIndex] = false;
        }
        //change section
        SamanGame_SaveLoad.SaveProgress();
    }
    ...
    public void SamanUniformGame_SelectCategory(int index)
    {
        ...
        if (index == 0)
        {
            ...
            selectedItem = SelectedItem.Dress;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-130f, -50f, 300f), 1);
        }
        else if (index == 1)
        {
            uIElements.HairScroller.SetActive(true);
            selectedItem = SelectedItem.Hair;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-85f, -460f, -400f), 1);
        }
        else if (index == 2)
        {
            uIElements.NecklaceScroller.SetActive(true);
            selectedItem = SelectedItem.Necklace;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-50f, -500f, -500f), 1);
        }
        else if (index == 3)
        {
            uIElements.EarringScroller.SetActive(true);
            selectedItem = SelectedItem.Earrings;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-90f, -230f, -270f), 1);
        }
        ...
        else if (index == 4)
        {
            ...
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-210f, 100f, 360f), 1);
        }
        
        else if (index == 5)
        {
            uIElements.bindiScroller.SetActive(true);
            selectedItem = SelectedItem.Bindi;
            uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-10f, -435f, -590.7f), 1);
        }
    ...
    public void SamanUniformGame_Submit()
    {
        uIElements.coinSlot.SetActive(false);
        uIElements.videoSlot.SetActive(false);
        uIElements.AllScrollers.SetActive(false); uIElements.PlayerCharacter.transform.DOLocalMove(new Vector3(-400f, -50f, 500f), 0.5f).OnComplete(delegate
        {
    ...
    IEnumerator SamanUniformGame_Judgement()
    {
        ...
        uIElements.playerPoints.text = ((((earringsRank + hairsRank + dressRank) * 100) / 30)).ToString();
        uIElements.opponentPoints.text = OP_MakeupPoints.ToString();
        StartCoroutine(SamanUniformGame_AddPlayerScore((((hairsRank + dressRank) * 100) / 30)));
        StartCoroutine(SamanUniformGame_AddOpponentScore(OP_MakeupPoints));
        ManageStars((((earringsRank + hairsRank + dressRank) * 100) / 30), OP_MakeupPoints);
        yield return new WaitForSeconds(2.5f);
    ...
    private void SamanUniformGame_DressupOpponent()
    {
        ...
        {
            OP_MakeupPoints++;
            WearItem(uIElements.OpponentBindi, BindiSprites);
        }
        OP_MakeupPoints++;
    ...
    public void SamanUniformGame_UnlockAllItems()
    {
        ...
        SamanUniformGame_UnlockList(bindiList);
